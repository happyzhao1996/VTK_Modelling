...
# Initialize
file_out_name = 'skyrmion_square.vtk'
...
for i in x:
	for j in y:
		...
		if i**2+j**2<=2500:
			if i!= 0:
				rotation_angle = [-(i**2+j**2)**0.5/50*180,-m.atan(j/i)/m.pi*180+np.sign(i)*90+180,0]
			else:
				rotation_angle = [-(i**2+j**2)**0.5/50*180,np.sign(j)*90+90,0]
		else:
